...
export default function Home({ data }: { data: Record<string, RowTrimmed> }) {
  ...
  const columns = [
    {
      ...
      field: "drug",
      renderCell: (row: any) => (
        <Link href={`/drug/${row.id}`}>{row.value}</Link>
      ),
      flex: 2,
    },
    {
  ...
  const rows = Object.values(data).filter((row) => {
    return (
      row.drug.toLowerCase().includes(filterValue) ||
      row.generic.toLowerCase().includes(filterValue) ||
      row.manufacturer.toLowerCase().includes(filterValue) ||
      row.id.includes(filterValue)
    );
  });
  ...
  return (
    <main>
      ...
      >
        The Prescription Affordability Navigator offers cost-saving suggestions
        for medications used in the treatment of <b>Asthma</b> and{" "}
        <b>Diabetes</b>
      </Box>
    </main>
  );
